...
- เมื่อ module มาจาก OSS ภายนอกจริง (Horilla, InvenTree ตามที่วางแผนไว้ใน [OSS-STRATEGY.md](../OSS-STRATEGY.md)) ซึ่งเป็นคนละภาษา/รันเป็น service แยก อยู่แล้ว (Python/Django) — module เหล่านั้น**ไม่ได้ใช้ mechanism นี้เลย** ต้องมี adapter แบบอื่น (HTTP proxy + service identity ที่มีอยู่แล้วจาก M2) ซึ่ง `orva-module-sdk` ในรูปแบบปัจจุบัน (Rust trait ตรง ๆ) ใช้ไม่ได้กับ module แบบนั้น — เป็น known gap ที่ต้องออกแบบเพิ่มตอนเริ่มต่อ OSS module จริง

...
| **G2 — The owner's inbox** ✅ code 2026-09-10 (owner: Gmail + Resend key) | tools, email→ticket, brief notification (2026-09-05); reply by email through `messages`, subject threading `[TCK-…]`, ticket merge, AI launcher moved to a corner icon (2026-09-10, child spec `2026-09-10-orva-support-inbox-and-brief.md`) | done | owner: connect Gmail (inbound) + set `RESEND_API_KEY` (outbound + brief digest) | reply-by-email + merge proven in the ephemeral suite with the captured email; J-003 end to end still needs the mailbox |

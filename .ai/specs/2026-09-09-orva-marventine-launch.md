...
| 2026-09-09 | **Narrowed on the real tenant (dev server, live walk):** `/api/customers/companies` returned `cf_th_tax_id` for both customer companies, matching `custom_field_values` row for row — so the list decorator does read stored values on the real tenant, where the definitions are tenant-scoped. The null reads were seen only in the ephemeral app, whose definitions are tenant-null (installed at build time, before any tenant exists). The gap is therefore *global definitions + in-app read*, still unlocated upstream; Orva's column readers stay because a label must not depend on which way the field was installed. The same walk rendered the 24-label sheet, the ใบส่งของ of KK-INV-2026012 (with its real payment line) and the ใบสั่งซื้อ sample with no console or server errors |

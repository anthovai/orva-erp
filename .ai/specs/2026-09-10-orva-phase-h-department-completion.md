...
**Date**: 2026-09-10
**Status**: **Complete 2026-09-10** — H1–H4 built, 56/56 integration tests green, and every screen walked on the real tenant. What remains is owner setup and the two flagged decisions (A3, A8), not code.
...
| 2026-09-10 | **H3 + H4 ephemeral green, then verified on the real tenant.** Final run 56 passed / 0 failed (the two new specs had needed a per-line `currencyCode` on the quote fixture). Migrations applied to the Kaiser tenant on dev-server boot in 12.7s: 6 new tables, 3 new columns, `orva_apply_rls()` confirmed ENABLE+FORCE with the `orva_tenant_isolation` policy on all six. `entities install` added the 3 consent fields (fields +3); `seed:defaults --module orva_support` registered the retainer scan (cron 30 6 * * *). Walked every new screen signed in: การตลาด reads the 2 real contacts with names decrypted, นำเข้าออเดอร์ reads the real brands ANT/MRV, โปรเจกต์ shows KK-QTN-2026011 with "ยังไม่ตั้งอัตราต่อชั่วโมง" rather than a false margin, บทความ round-tripped a throwaway article (Thai slug intact) that was then removed from the table, and the portal route refused a staff session with 401. 13 client-side navigations including the previously-crashing expenses screen: no console error. **Found and fixed**: `orva_marketing`'s `defaultRoleFeatures` had reached no role — `seed:defaults` does not apply them and the superadmin account bypasses feature checks, so nothing showed; `yarn mercato auth sync-role-acls` granted `orva_marketing.*` to admin and `.view` to employee. Lesson recorded |

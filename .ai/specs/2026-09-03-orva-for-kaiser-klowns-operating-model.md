...
| E — shipped 2026-09-03 | Upstream `wms` re-enabled (zones/reservations/dashboard hidden) for lots, expiry, balances, movements; catalog custom fields `th_fda_notification`, `shelf_life_months`, `product_brand` (edit screen); new app module `orva_stock`: cost per lot (`orva_stock_lot_costs`) from the OEM bill line, `POST /api/orva_stock/receive` (WMS receive + cost), `POST /api/orva_stock/retail-sale` (brand-series invoice, record-payment, WMS adjust, COGS queue), `GET valuation`, `GET/POST cogs` (Dr 5010 / Cr 1200 monthly, idempotent); screens รับสินค้าเข้าคลัง / ขายปลีก / สินค้าคงเหลือและต้นทุน under menu "สินค้า Marventine". Documents gain `abbreviated_tax_invoice` (ใบกำกับภาษีอย่างย่อ: buyer optional, VAT-inclusive prices, T05). Baseline configured on the tenant: warehouse MAIN / location A-01, accounts 1200 สินค้าคงเหลือ and 5010 ต้นทุนขายสินค้า. Untested end-to-end until the first real product/lot exists | when the product ships |

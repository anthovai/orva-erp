...
| Q4 | QR code | **Resolved 2026-09-08 — owner approved the dependency.** `qrcode` (MIT, and every package in its tree is MIT or ISC) renders the enrollment QR **on the server** in `lib/qr.ts`, so the browser bundle does not grow by an encoder and the client never holds the otpauth URI to draw it. A render failure returns null and the screen falls back to the key plus the `otpauth://` link, which is exactly what it showed before. Covered by 2 unit tests and 3 integration specs against a production build | AGENTS: ask before dependencies |

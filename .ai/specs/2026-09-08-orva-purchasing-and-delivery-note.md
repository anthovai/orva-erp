...
- **Exit gate:** **met, in two steps.** The sheet's rules are proven by the render tests — heading, delivery block, two dated signature lines, price hiding on both eligible templates, and the fallback when a wrong template is forced. The sheet itself was then walked in a browser by B2's integration spec: the preview page loads with a real session against a production build, the printed text states the goods and the delivery and no prices, and the page raises no client-side error. The **public link** was closed on 2026-09-09 by `share-document` + `orva_documents_share_links` (see Migration & Backward Compatibility and the changelog): a delivery note is minted its own link, the public page opens it with no session and no prices, minting again revokes the earlier link, and a tax type is refused at the schema. The **emailed PDF** needs no new code — the send route already accepts `type=delivery_note` and builds its subject from `headingTh` — but cannot be exercised in the harness (no provider key); it is exercised wherever `RESEND_API_KEY` is set.
...
- **The brand and compact forms are refused, twice** — with a correction dated 2026-09-09: `templateFor` will not choose them, and `templateComponentFor` overrides them, but the preview **page** called the chooser with only the payslip flag until the G3 rehearsal's screenshot showed a label sheet rendered as an invoice; the page now passes the whole document, and the delivery-note browser spec forces `template=brand` on the URL to prove it. Reason for the refusal: `brand` is the tenant's own tax-invoice form with its **own** money table outside the shared blocks (it would leak prices), and `compact` has no signature block (an unsignable delivery note).
...
## 📝 Migration & Backward Compatibility (share links, 2026-09-09)

The public link for a ใบส่งของ adds one table, one route and one optional response field. Every change is additive under `.ai/guides/upstream/BACKWARD_COMPATIBILITY.md`:

| Surface | Change | Classification |
|---|---|---|
| Database schema | New table `orva_documents_share_links` (token hash unique, `(tenant_id, organization_id)` and `(source_kind, source_id)` indexes, `orva_apply_rls()`); migration `Migration20260909130000_document_share_links`, dry-run valid, applied to the tenant 2026-09-09 | ✓ ADDITIVE (new table) |
| API routes | New `POST /api/orva_documents/share-document` `{ documentId, type ∈ SHAREABLE_TYPES, expiresInDays? }` → `{ url, validUntil }`, feature `sales.invoices.manage` | ✓ ADDITIVE (new route) |
| API response | `GET /api/orva_documents/public/[token]` gains optional `kind: 'quote' \| 'share_link'`; existing fields unchanged; the quote path answers byte-for-byte as before plus that field | ✓ ADDITIVE (optional response field) |
| Public route behaviour | A token that matches no quote is now looked up in share links before answering 404. Tokens that matched a quote before still match the quote first | ✓ Behaviour-preserving for every existing link |
| `lib/document.ts` exports | New `SHAREABLE_TYPES` and `isShareable()`; nothing renamed | ✓ ADDITIVE |
| `data/validators.ts` | New `shareDocumentSchema`; `shareSchema` untouched | ✓ ADDITIVE |
| CLI | New `mercato orva_party add-party` / `list-parties` | ✓ ADDITIVE |
| Schedule ids | `lateScanScheduleId` / `overdueScanScheduleId` / `dailyBriefScheduleId` keep their names and signatures but now return a uuid derived from the readable key; `seedDefaults` resolves an existing row for the queue first, so the two finance schedules already on the tenant keep their ids | ✓ Behaviour-preserving (verified: re-seeding both modules left exactly three rows) |

**Migration path for existing tenants**: `yarn db:migrate` (the new table), then `yarn mercato seed:defaults --module orva_purchasing` (registers the late scan — which had never succeeded before this fix, see the changelog), and `yarn mercato auth sync-role-acls --tenant <id>` to grant `orva_purchasing.*` to roles created before the module existed (merge-only; verified admin 126→130, employee 106→111, superadmin 54→58 features, nothing removed).
...
| 2026-09-09 | **Public link for ใบส่งของ**: `orva_documents_share_links` (token hash only, expiry, revocation, RLS), `POST /api/orva_documents/share-document`, the public route's second door (`kind: 'share_link'`), `SHAREABLE_TYPES = ['delivery_note']` decided in `lib/document.ts`, the preview page's existing button reused for a delivery note printed from an invoice. Migration dry-run valid and applied. 2 integration specs: mint → anonymous read → rotation kills the old token → tax type refused; the public page opened in a browser with no session and no prices |
| 2026-09-09 | **Bug (A4, latent since shipping):** the late-delivery scan was never registered on the real tenant — `schedulerService.register` upserts by a uuid `id` and both `orva_purchasing` and `orva_finance` passed a readable key, which Postgres rejects; `seedDefaults` swallowed it as a warning. Fixed with `src/lib/scheduleId.ts` (name-based uuid + resolve-existing-row) in both modules; re-seeding both left exactly three schedule rows, finance's two keeping their ids. Recorded as a lesson |
| 2026-09-09 | **The real tenant can now use purchasing**: `mercato orva_party add-party` (new CLI, the route's twin for a terminal-only tenant) created the first vendor, `OEM Marventine (ยังไม่ระบุชื่อบริษัท)` — a deliberately provisional name with no tax id, to be renamed in ทะเบียนคู่ค้า once the OEM is chosen; `auth sync-role-acls` granted `orva_purchasing.*` (merge-only, verified); `seed:defaults` registered the 06:30 scan |
